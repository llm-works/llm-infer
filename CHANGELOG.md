...
- **Model warmup with EOS verification**: base model and LoRA adapters warmed up with token sweep.
  Adapters flagged if they hit max_tokens where base model produced EOS (indicates training issue).
  Note: EOS verification only works with vLLM-server engine.
